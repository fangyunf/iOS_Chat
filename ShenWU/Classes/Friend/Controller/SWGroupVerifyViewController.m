...
@interface SWGroupVerifyViewController ()
...
@implementation SWGroupVerifyViewController
...
//    bgImgView.image = [UIImage imageNamed:@"bg_mine"];
    ...
    [self.avatarImgView sd_setImageWithURL:[NSURL URLWithString:self.model.userAvatar] placeholderImage:[UIImage imageNamed:@"avatar_person"]];
    [self.view addSubview:self.avatarImgView];
    
    self.nameLabel = [FControlTool createLabel:self.model.userName textColor:UIColor.blackColor font:[UIFont boldFontWithSize:15]];
    ...
    self.idLabel = [FControlTool createLabel:[NSString stringWithFormat:@"ID:%@",self.model.userMemberCode] textColor:UIColor.blackColor font:[UIFont fontWithSize:12]];
    ...
    UILabel *groupTitleLabel = [FControlTool createLabel:@"加人的群聊" textColor:UIColor.blackColor font:[UIFont boldFontWithSize:14]];
    ...
    self.remarkTextField.text = self.model.groupName;
    ...
    agreeBtn.frame = CGRectMake(60, remarkBgView.bottom+20+21+74+58, kScreenWidth - 120, 52);
    agreeBtn.backgroundColor = kMainColor;
...
- (void)refuseBtnAction{
    if (self.delegate && [self.delegate respondsToSelector:@selector(groupRefuseApply:)]) {
        [self.delegate groupRefuseApply:self];
    }
    [self handleFriendApply:2];
}

- (void)agreeBtnAction{
    if (self.delegate && [self.delegate respondsToSelector:@selector(groupAgreeApply:)]) {
        [self.delegate groupAgreeApply:self];
    }
...
- (void)handleFriendApply:(NSInteger)type{
    @weakify(self)
    [SVProgressHUD show];
    NSDictionary *params = @{@"id":self.model.applyId,@"state":@(type)};
    [[FNetworkManager sharedManager] postRequestFromServer:@"/group/groupConsentOrRefuse" parameters:params success:^(NSDictionary * _Nonnull response) {
