...
- (void)viewDidLoad
{
    ...
    [[NSNotificationCenter defaultCenter] addObserver:self selector:@selector(startScanNotication:) name:@"VKStartScanNotication" object:nil];
}
...
- (void)startScanNotication:(NSNotification*)note {
    [self reStartDevice];
}
...
- (void)drawBottomItems
{
    ...
    _btnFlash.center = CGPointMake(CGRectGetWidth(_bottomItemsView.frame)/2 + size.width, CGRectGetHeight(_bottomItemsView.frame)/2);
...
//    NSDictionary *dict = @{@"type" : @(QRCodeType_person).stringValue, @"cid" : ShopApp.user.user_id};
//    [QRCodeDisplayVC ShowVC:dict];
}
@end
