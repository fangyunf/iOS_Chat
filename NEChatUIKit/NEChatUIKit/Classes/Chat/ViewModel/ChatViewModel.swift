...
open class ChatViewModel: NSObject, ChatRepoMessageDelegate, NIMChatManagerDelegate,
  ...
  public var messages = [MessageModel]()
    public var tempMessages = [NIMMessage]()
  ...
  init(session: NIMSession) {
    ...
    repo.addChatDelegate(delegate: self)
      repo.removeContactDelegate(delegate: self)
    repo.addContactDelegate(delegate: self)
      repo.removeSessionDelegate(delegate: self)
    repo.addSessionDelegate(delegate: self)
      repo.removeSystemNotificationDelegate(delegate: self)
    repo.addSystemNotificationDelegate(delegate: self)
      repo.removeChatExtendDelegate(delegate: self)
    repo.addChatExtendDelegate(delegate: self)
      repo.removeTeamDelegate(delegate: self)
    repo.addTeamDelegate(delegate: self)
    addObserver()
      msgTimer = Timer(timeInterval: 1.0, target: self, selector: #selector(refreshMsg), userInfo: nil, repeats: true)
      RunLoop.current.add(msgTimer!, forMode: .common)
  }
  ...
  init(session: NIMSession, anchor: NIMMessage?) {
    ...
    repo.addTeamDelegate(delegate: self)
    addObserver()
      msgTimer = Timer(timeInterval: 0.2, target: self, selector: #selector(refreshMsg), userInfo: nil, repeats: true)
      RunLoop.current.add(msgTimer!, forMode: .common)
  }
...
//              self.tempMessages.append(msg)
    ...
    @objc func refreshMsg(){
        if self.tempMessages.count > 0 {
            let msg:NIMMessage = self.tempMessages[0]
            self.tempMessages.removeFirst()
            let model = modelFromMessage(message: msg)
            ChatMessageHelper.addTimeMessage(model, self.messages.last)
            self.messages.append(model)

            delegate?.onRecvMessages([msg])
        }
    }
