...
open class NEBaseSelectUserViewController: ChatBaseViewController, UITableViewDelegate,
  ...
  private var showSelf = true // 是否展示自己
    @objc public var searchUsers: [NEChatKit.ChatTeamMemberInfoModel] = [NEChatKit.ChatTeamMemberInfoModel]()
  ...
  func commonUI() {
    ...
    NSLayoutConstraint.activate([
      ...
      searchView.frame = CGRectMake(15, kNavigationHeight + 6, kScreenWidth - 30, 32)
      searchView.backgroundColor = HexRGB(0xF3E8DB)
      searchView.layer.cornerRadius = 16
      searchView.layer.masksToBounds = true
      view.addSubview(searchView)
      
      searchTextField.frame = CGRectMake(15, 0, searchView.width - 30, 32)
  ...
  open func tableView(_ tableView: UITableView, didSelectRowAt indexPath: IndexPath) {
    ...
    if let block = selectedBlock {
        if isSearch {
            block(indexPath.row, searchUsers[indexPath.row])
        }else{
...
extension NEBaseSelectUserViewController:UITextFieldDelegate{
    ...
    public func textField(_ textField: UITextField, shouldChangeCharactersIn range: NSRange, replacementString string: String) -> Bool {
        let changedString:NSMutableString = NSMutableString.init(string: textField.text ?? "")
        changedString.replaceCharacters(in: range, with: string)
        ...
        if string == "\n" {
            if searchTextField.text != nil && searchTextField.text!.count > 0 {
                isSearch = true
                searchUsers.removeAll()
                if teamInfo != nil  {
                    for item in teamInfo!.users {
                        if item.showNickInTeam().lowercased().contains(searchTextField.text!.lowercased()) {
                            searchUsers.append(item)
                        }
                    }
                }
                tableView.reloadData()
