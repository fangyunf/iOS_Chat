...
open class ChatViewController: ChatBaseViewController, UINavigationControllerDelegate,
  ...
  UITableViewDelegate, UIDocumentPickerDelegate, UIDocumentInteractionControllerDelegate, CLLocationManagerDelegate, UITextViewDelegate, ChatInputMultilineDelegate, UIScrollViewDelegate {
  ...
  open func setMoreButton() {
    ...
    open func setNavTitle(name:String) {
        title = name
    }
  open func commonUI() {
      view.backgroundColor = HexRGB(0xF5F7FA)
...
//        view.image = UIImage(named: "bg_msg")
      return view
    }()
  ...
  public lazy var contentView: UIView = {
    ...
    content.backgroundColor = UIColor.clear
      content.addSubview(bgImgView)
    content.addSubview(tableView)
      
      NSLayoutConstraint.activate([
        bgImgView.topAnchor.constraint(equalTo: content.topAnchor),
        bgImgView.leftAnchor.constraint(equalTo: content.leftAnchor),
        bgImgView.rightAnchor.constraint(equalTo: content.rightAnchor),
        bgImgView.bottomAnchor.constraint(equalTo: content.bottomAnchor),
      ])
  ...
  public lazy var bottomView: UIView = {
    ...
    NSLayoutConstraint.activate([
      ...
      chatInputView.heightAnchor.constraint(equalToConstant: 504),
      chatInputView.topAnchor.constraint(equalTo: view.topAnchor),
    ])
      
//      chatInputView.moreItemDatas = NEChatUIKitClient.instance.getMoreActionData(sessionType: viewmodel.session.sessionType)
//      chatInputView.setupMoreMenu()
  ...
  deinit {
    ...
    cleanDelegate()
      viewmodel.msgTimer?.invalidate()
      viewmodel.msgTimer = nil
  }
  ...
  //    MARK: ChatInputViewDelegate
    public func bottomMoreAction(item: NEMoreItemModel) {
        moreBtnAction(item: item)
    }
  ...
  open func didSelectMoreCell(cell: NEInputMoreCell) {
    ...
    if let type = cell.cellData?.type, type == .location {
      if #available(iOS 14.0, *) {
        ...
        if CLLocationManager.authorizationStatus() == .denied {
          showSingleAlert(message: commonLocalizable("x")) {}
          return
      ...
      didToSearchLocationView()
        ...
        if #available(iOS 14.0, *) {
            ...
            // 设置选择的限制条件
            configuration.selectionLimit = 9 // 最多允许选择的数量，默认值为1
    ...
    } else if let type = cell.cellData?.type, type == .collect {
          
    } else if let type = cell.cellData?.type, type == .card {
     
    } else if let type = cell.cellData?.type, type == .quan {
    ...
    open func moreBtnAction(item:NEMoreItemModel){
        if item.type == .takePicture {
            isFile = false
            showTakePicture()
            ...
            if let delegate = item.customDelegate as? AnyObject, let action = item.action {
              // 用户自定义更多面板按钮
              _ = delegate.perform(action)
              return
            }
  ...
  open func willSelectItem(button: UIButton?, index: Int) {
    ...
    if index == 2 || button?.isSelected == true {
      if index == 0 {
        ...
        // 更多
        layoutInputView(offset: bottomExanpndHeight)//bottomExanpndHeight
        scrollTableViewToBottom()
      }
    } else {
      layoutInputView(offset: 0)
  ...
  open func startRecord() {
    ...
    if NEAuthManager.hasAudioAuthoriztion() {
        NIMSDK.shared().mediaManager.recordProgressUpdateTimeInterval = 1.0
  ...
  open func recordAudio(_ filePath: String?, didBeganWithError error: Error?) {
    print("[record] sdk Began error:\(error?.localizedDescription ?? "")")
      NotificationCenter.default.post(name: NSNotification.Name("startRecordAudio"), object: nil)
  }
  ...
  open func recordAudio(_ filePath: String?, didCompletedWithError error: Error?) {
    ...
    if dur > 1 {
      ...
      NotificationCenter.default.post(name: NSNotification.Name("endRecordAudio"), object: nil)
  }
  ...
  open func recordAudioProgress(_ currentTime: TimeInterval) {
      NotificationCenter.default.post(name: NSNotification.Name("recordAudioProgress"), object: currentTime)
  }
...
//        let isAtBottom = visibleCellsCount > 0 && contentHeight - tableView.contentOffset.y - tableView.bounds.height < 10
        if self.isBottom {
  ...
  open func copyMessage() {
      if let model = viewmodel.operationModel as? MessageTextModel {
        ...
        if let text = model.message?.text, !text.isEmpty {
            let str = MessageTextModel.aes_decrypt(text, aes_key: "wallstreetimchat")
            let copyStr = str.count == 0 ? text : str
          ...
          showToast(chatLocalizable("copy_success"))
            ...
            let str = MessageTextModel.aes_decrypt(body, aes_key: "wallstreetimchat")
            let copyStr = str.count == 0 ? body : str
          UIPasteboard.general.string = copyStr
          showToast(chatLocalizable("copy_success"))
          ...
          if let title = model.titleAttributeStr?.string, !title.isEmpty {
              let str = MessageTextModel.aes_decrypt(title, aes_key: "wallstreetimchat")
              let copyStr = str.count == 0 ? title : str
  ...
  open func tableView(_ tableView: UITableView,
      ...
      if m.type == .custom {
          if let object = m.message?.messageObject as? NIMCustomObject, let custom = object.attachment as? NECustomAttachmentProtocol {
              if custom.customType == 10086 {
                  return 104 + chat_content_margin
              }
              return custom.cellHeight+chat_content_margin
          }
      }
      return m.cellHeight()+chat_content_margin
...
//          if let object = model.message?.messageObject as? NIMCustomObject, let custom = object.attachment as? NECustomAttachmentProtocol {
//              if custom.customType == 10086 {
//                  return 104 + chat_content_margin
//              }
//              return custom.cellHeight + chat_content_margin
...
extension ChatViewController: ChatBaseCellDelegate {
  open func didLongPressAvatar(_ cell: UITableViewCell, _ model: MessageContentModel?) {
    ...
    open func addToAtUserAction(model:MessageContentModel?) {
        var addText = ""
        var accid = ""
