...
//    public var selectType:NIMSessionType = .P2P
  ...
  open func sortRecentSession() {
    ...
    conversationListArray = sortResultArr
      ...
      var aideNewsModel:ConversationListModel?
      var aideNewsIndex = 0
      if let array = conversationListArray {
        for index in 0 ..< array.count {
          let m = array[index]
            if m.recentSession?.session?.sessionId == aideNewsUserId {
                aideNewsModel = m
                aideNewsIndex = index
          }
        }
      }
      if aideNewsModel != nil {
          conversationListArray?.remove(at: aideNewsIndex)
          conversationListArray?.insert(aideNewsModel!, at: 0)
      }
      
      
      
//      var stickTopResultArr = [ConversationListModel]()
//      var tempArr = [ConversationListModel]()
//      var indexArr = [Int]()
//      conversationListArray?.forEach { listModel in
//          tempArr.append(listModel)
//      }
//      
//      if conversationListArray != nil {
//          for (index,listModel) in tempArr.enumerated() {
//              if let _ = stickTopInfos[(listModel.recentSession?.session)!] {
//                  indexArr.append(index)
//                  stickTopResultArr.append(listModel)
//              }
//          }
//          
//          for index in indexArr.reversed() {
//              conversationListArray?.remove(at: index)
//          }
//      }
//      
//      conversationListArray?.insert(contentsOf: stickTopResultArr, at: 0)
...
//      if recentSession.session?.sessionType != selectType {
//          return
//      }
    ...
    if recentSession.session?.sessionType == .P2P {
        repo.fetchUserInfo(accountList: [targetId]) { [self] users, error in
        if error == nil {
          listModel.userInfo = users?.first
          if let model = weakSelf?.sessionIsExist(listModel) {
            model.userInfo = users?.first
          } else {
              if weakSelf?.conversationListArray?.count ?? 0 > self.stickTopInfos.count {
                  weakSelf?.conversationListArray?.insert(listModel, at: self.stickTopInfos.count)
              }else{
...
//              weakSelf?.conversationListArray?.append(listModel)
      ...
      if !repo.isMyTeam(teamId: targetId) {
        ...
        repo.getTeamInfo(teamId: targetId) { [self] error, teamInfo in
        listModel.teamInfo = teamInfo
        if let model = weakSelf?.sessionIsExist(listModel) {
          ...
          if let _ = conversationListArray {
              for i in 0 ..< conversationListArray!.count {
                let listModel = conversationListArray![i]
                  if listModel.recentSession?.session?.sessionId == recentSession.session?.sessionId {
                      conversationListArray?.remove(at: i)
                      conversationListArray?.insert(listModel, at: 0)
                      break
                  }
              }
          }
