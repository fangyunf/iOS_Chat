...
open class NEBaseConversationController: UIViewController, NIMChatManagerDelegate, UIGestureRecognizerDelegate {
  ...
  public var deleteBottonBackgroundColor: UIColor = NEConstant.hexRGB(0xA8ABB6)
    public var isAppear = true
    public var brokenNetworkViewHeight = 0.0
  ...
  public var popListView = NEBasePopListView()
    public var selectIndex = 100;
    public var selectType:NIMSessionType = .P2P
    public var isFilterType:Bool = false
    public var serviceUserId = "";
    public var searchContent = "";
    public var aideNewsUserId = "";
    public var isCanReload = true;
  ...
  override open func viewDidLoad() {
    ...
    NotificationCenter.default.addObserver(self, selector: #selector(didRefreshTable), name: NENotificationName.updateFriendInfo, object: nil)
      NotificationCenter.default.addObserver(self, selector: #selector(canReload), name: UIApplication.didBecomeActiveNotification, object: nil)
      NotificationCenter.default.addObserver(self, selector: #selector(cannotReload), name: UIApplication.didEnterBackgroundNotification, object: nil)
  }
...
//      }
  ...
  func requestData() {
    ...
    viewModel.fetchServerSessions(option: params) { error, recentSessions in
      if error == nil {
          ...
          if recentList.count > 0 {
            ...
            weakSelf?.delegate?.onDataLoaded()
              ...
              NELog.infoLog(
                ...
                desc: "❌CALLBACK fetchServerSessions empty"
              )
        ...
        NotificationCenter.default.post(name: NSNotification.Name("checkGroup"), object: nil)
    }
  }
  ...
  public  func filterConversationList() -> [ConversationListModel] {
        if isFilterType {
            if searchContent.count > 0 {
                if selectType == .team {
                    return viewModel.conversationListArray?.filter({$0.recentSession?.session?.sessionType == selectType && ( ($0.teamInfo?.teamName?.matchSearch(searchContent)) == true)}) ?? [ConversationListModel]()
                }else{
                    return viewModel.conversationListArray?.filter({$0.recentSession?.session?.sessionType == selectType && ( ($0.userInfo?.showName()?.matchSearch(searchContent)) == true)}) ?? [ConversationListModel]()
                }
              
            }else{
                return viewModel.conversationListArray?.filter({$0.recentSession?.session?.sessionType == selectType}) ?? [ConversationListModel]()
            }
        }else{
            if searchContent.count > 0 {
                return viewModel.conversationListArray?.filter({( ($0.userInfo?.showName()?.matchSearch(searchContent)) == true || ($0.teamInfo?.teamName?.matchSearch(searchContent)) == true)}) ?? [ConversationListModel]()
            }else{
                return viewModel.conversationListArray ?? [ConversationListModel]()
            }
        }
...
extension NEBaseConversationController: UITableViewDelegate, UITableViewDataSource {
  open func tableView(_ tableView: UITableView, numberOfRowsInSection section: Int) -> Int {
      let count:Int = filterConversationList().count
  ...
  open func tableView(_ tableView: UITableView,
      ...
      let model:ConversationListModel? =  filterConversationList()[indexPath.row]
  ...
  open func tableView(_ tableView: UITableView, didSelectRowAt indexPath: IndexPath) {
      let conversationModel:ConversationListModel? = filterConversationList()[indexPath.row]
...
extension NEBaseConversationController: ConversationViewModelDelegate {
  ...
  open func didUpdateRecentSession(index: Int) {
    let indexPath = IndexPath(row: index, section: 0)
      if tableView.isEditing == false  && isCanReload == true {
          if isAppear {
